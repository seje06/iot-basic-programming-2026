...
- Git, Github, Desktop 설치
    - https://git-scm.com/ 에서 install for windows 다운로드 후 설치
    - https://desktop.github.com/download/ 다운로드 후 설치
...
### 프로그램 개요
- 프로그램이란 : 컴퓨팅환경(PC, 모바일, 임베디드 등)에서 `사용자의 요구`에 따라 `데이터를 처리`(표현, 이동, 저장, 로드..)을 할 수 있는 `소프트 웨어`
...
1. 기본 문법
    ...
    - 입력 : [확인](./Day02/Chap03-Solution/Chap03-03-app/main.c)
...
2. 반복문
    ...
    - for 문 : [확인](./Day03/Chap05-Solution/Chap05-04-app/main.c)
...
2. 배열 : 
    - 동일 자료형의 묶음 - `ary[10]`
    - 배열 선언, 초기화 : [확인](./Day04/Chap08-Solution/Chap08-01-app/)
    - 문자배열(문자열) : [확인](./Day04/Chap08-Solution/Chap08-03-app/)
...
1. 포인터
    ...
    - 포인터 이해 : [확인](./Day05/Chap09-Solution/Chap09-02-app/main.c)
        ...
        - 변수의 주소는 바뀌지 않는 값&(상수), 변수의 주소(상소)는 바꿀 수 없다.
    ...
    - 포인터 사용 이유 : [소스](./Day05/Chap09-Solution/Chap09-05-app/main.c)
        ...
        - 함부로 변수를 마구 찍어서 만들 수 없음(용량 문제 발생)!
...
2. 배열과 포인터 : [소스](./Day05/Chap10-Solution/Chap10-01-app/main.c)
    ...
    - int ary[100] -> main에서 400byte, 함수에서 8byte를 메모리공간 확보
    - 배열의 주소를 함수에 전달 : [소스](./Day05/Chap10-Solution/Chap10-03-app/main.c)

3. 문자 : [소스](./Day05/Chap11-Solution/Chap11-01-app/main.c)
    - getchar() 입력버퍼 한글자 키우기 : [소스](./Day05/Chap11-Solution/Chap11-02-app/main.c)
...
4. 문자열과 포인터 : [소스](./Day06/Chap12-Solution/Chap12-01-app/main.c)
    ...
    - strcmp, strncmp : 문자열 비교함수. 두문자열이 같은지 확인
    - strcspn() : 문자열에서 찾고자 하는 문자열에 포함된 문자를 검색, 해당 문자의 위치를 반환
    - strtok() : 패턴문자열로 기존 문자열을 자르는 함수
    - strchr() : 문자열에서 특정 문자가 가장 먼저 나타나는 곳의 위치를 리턴하는 함수 

5. Tip
    - #pragma region, #pragma endregion : 소스코드 영역에서 일부 축소(확장)시키고자 할때

6. 변수 영역과 : [소스](./Day06/Chap13-Solution/Chap13-01-app/main.c)
    - `auto` : 지역변수를 뜻함. auto라는 키워드를 별로 사용안함.
    - global : 전역변수. global 단어를 사용하는게 아님. 전역변수는 문제가 많아 제한적으로 사용할 것
...
11. 함수와 포인터 : [소스](./Day07/Chap15-Solution/Chap15-01-app/main.c)
    ...
    - 사용 예 - 임베디드에서 하드웨어 이벤트 인터럽트, 상태머신, GUI 이벤트, 게임 상태별 로직
...
3. 메모리 동적할당 : [소스](./Day07/Chap16-Solution/Chap16-01-app/main.c)
...
6. main 함수 명령행 인수 사용 : [소스](./Day07/Chap16-Solution/Chap16-06-app/main.c)
    - int main(int argc, char** argv) 가 기본 메인함수 형태
    - 프로젝트 속성에서 명령인수 입력방법
...
2. 폴더 및 파일경로
    ...
    - 상대경로 - `./LICENSE.rtf` 와 같이, .(현재폴더), ..(부모폴더) 기호를 사용해서 짧게 경로를 표시
        - 장점 - 경로를 짧게 표시.
        - 단점 - 개발자가 직접 경로가 어떤식으로 구성되는지 파악필요. 같은 폴더에 존재하는 경우 사용 가능
...
3. 파일 입출력 : [소스](./Day09/Chap18-Solution/Chap18-01-app/main.c)
    ...
    - fseek(). rewind() - 파일에 처음으로 돌가아서 다시 읽기
    - feof() - 파일에 끝까지 다읽었는지 확인하는 함수
    - `fgets()` - 파일의 커서(현재위치)에서 한줄을 읽는 함수
    - `fputs()` - 파일에 한줄을 출력(쓰는) 함수
    - fscanf() - 포맷에 맞춰서 파일에 쓰기 함수
    - fprintf() - 포맷에 맞춰서 파일에 쓰기 함수
    - stdio.h - stdin(표준입력), stdout(표준출력), stderr(표준에러) 포함
    - 인코딩 방식 - `EUC-KR`(CP-949, 멀티바이트, 한글만 표현, 2byte ), `UTF-8`(한글자 최대 6byte, 한글은 3byte)
...
7. 전처리 지시자 : [소스](./Day09/Chap19-Solution/Chap19-01-app/main.c)
    - #include - 파일 포함 지시자
    - #define - 새로 정의하는 지시자. 자주 사용될 상수값을 정의할 때 많이 사용. 주로 대문자로 작성
