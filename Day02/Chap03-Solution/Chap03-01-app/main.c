...
int main(void)
{
	...
	// ASCII 영문자, 특수문자(+,-,etc) 등 숫자값으로 1:1 매칭되어있음
	// https://ko.wikipedia.org/wiki/ASCII 아스키표 참조

	// 정수형
	// char : 크기 1byte -> 8bit 00000000 -> 00~FF (0~255 까지 표현) 최대크기 255
	// short : 2bytes 
	// int(기본, 제일중요) : 4bytes 
	// long : 4bytes
	// long long : 8bytes
	...
	printf("변수 long, %zu byte, 최소 %lld, 최대 %lld\n", sizeof(long), LLONG_MIN, LLONG_MAX);

	// unsigned 양수로만 표현하는 방법
	// 최소값이 0
	printf("변수 unsigend char, %zu byte, 최소 %d, 최대 %d\n", sizeof(unsigned char), 0, UCHAR_MAX);
	printf("변수 unsigend char, %zu byte, 최소 %d, 최대 %d\n", sizeof(unsigned short), 0, USHRT_MAX);
	printf("변수 unsigend char, %zu byte, 최소 %d, 최대 %u\n", sizeof(unsigned int), 0, UINT_MAX);
	printf("변수 unsigend char, %zu byte, 최소 %d, 최대 %u\n", sizeof(unsigned long), 0, ULONG_MAX);
	printf("변수 unsigend char, %zu byte, 최소 %d, 최대 %llu\n", sizeof(unsigned long long), 0, ULLONG_MAX);
}