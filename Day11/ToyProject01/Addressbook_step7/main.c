...
#define INIT_CAPACITY   2	 // 초기값 15로 구성
...
static void add_contact(void) {
	...
	while (1)
	{
		...
		case 0:
			printf("이름[최대10자] : ");
			read_line(contacts[count].name, NAME_LEN); //이름에 파이프가 들어가면
			if (contains_pipe(contacts[count].name)) break;

			progress++;
		case 1:
			printf("전화[최대20자] : ");
			read_line(contacts[count].phone, PHONE_LEN);
			if(contains_pipe(contacts[count].phone)) break;
		...
		case 2:
			...
			if(contains_pipe(contacts[count].address)) break;

			progress++;
		case 3:
			printf("이메일 : ");
			read_line(contacts[count].email, EMAIL_LEN);
			if(contains_pipe(contacts[count].email)) break;

			progress++;
		case 4:
			printf("메모[최대10자] : ");
			read_line(contacts[count].memo, MEMO_LEN);
			if(contains_pipe(contacts[count].memo)) break;
...
static int load_contacts(const char* filename)
{
	...
	while (fgets(line, sizeof(line), fp) != NULL)
	{
		...
		contacts[count].phone[PHONE_LEN - 1] = '\0'; //이름 길이보다 길게 쓰면 정리

		strncpy(contacts[count].address, address, ADDR_LEN);
		contacts[count].address[ADDR_LEN - 1] = '\0'; //이름 길이보다 길게 쓰면 정리

		strncpy(contacts[count].email, email, EMAIL_LEN);
		contacts[count].email[EMAIL_LEN - 1] = '\0'; //이름 길이보다 길게 쓰면 정리

		strncpy(contacts[count].memo, memo, MEMO_LEN);
		contacts[count].memo[MEMO_LEN - 1] = '\0'; //이름 길이보다 길게 쓰면 정리
